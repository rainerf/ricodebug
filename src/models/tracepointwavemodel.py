...
class TracepointWaveScene(QtGui.QGraphicsScene): 
    '''QGraphicsView for wave'''
    def __init__(self, type, values, duration):
        '''CTOR
           @param type: string, must be in supported types
...
class TracepointWaveModel(QAbstractTableModel):
    ...
    def updateTracepointWave(self, name, type, value):
        '''Update existing tracepoint wave. If model does not contain a wave with given name and type a new wave is created.
            ...
            @param type: string, type of wave (must be supported by TracepointWaveScene)
            @param value: value to append to wave
        '''
        i = self.__getTracepointWaveIndex(name, type)
        ...
        if i != None:
            ...
            waveform.setScene(TracepointWaveScene(type, [value], self.duration))      
    ...
    def __getTracepointWaveIndex(self, name, type):
        for i in range(len(self.waveforms)):
            if self.waveforms[i].getName() == name and self.waveforms[i].scene().getType() == type:
